export const getUserResult = (answers, lives) => {
  ...
  if (lives < 0 || lives > LIVES_LIMIT) {
    throw new Error(`lives should be greater than 0 less than 4`);
  }
  ...
  let positiveArrayAnswers = answers.filter((round) => {
    if (typeof round !== `object`) {
      throw new Error(`Wrong type Expect answers index type`);
    }
    if (typeof round.answer !== `boolean`) {
      throw new Error(`Wrong type Expect answer key type`);
    }
    if (typeof round.time !== `number`) {
      throw new Error(`Wrong type Expect time key type`);
    }
